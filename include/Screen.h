...
public:
    Screen();
    void write(char *number1, char *number2);
    void update();
    ~Screen();
};
...
void Screen::update()
{
    ...
    char buffer2[4] = {0};
    char dem2[1] = {0};
    sprintf(buffer1, "%04.0f", txAzEl.az);
...
void Screen::write(char *number1, char *number2)
{
...
void Screen::resetDisplay()  
{
    ...
    set_register(MAX7219_REG_INTENSITY, 0x90); // display intensity
}
